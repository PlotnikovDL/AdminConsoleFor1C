...
- `Агенты сервера 1С` - службы Windows, агент сервера, управление запуском, остановкой и перезапуском, основные параметры службы.
- `Процессы` - процессы 1С (`ragent.exe`, `rmngr.exe`, `rphost.exe`, `ras.exe`, `dbgs.exe`), PID, роли, порты и параметры запуска.
...
Основной формат публичной установки: EXE-установщик Inno Setup с автономной (unpackaged, self-contained) сборкой WinUI 3. .NET и Windows App SDK входят в поставку.

Сборка MSIX сохранена как дополнительный вариант. Самоподписанный MSIX предназначен для тестирования и не используется в публичном каталоге WinGet.

Целевой канал распространения:

- GitHub Releases - публикация EXE-установщика и SHA256.
